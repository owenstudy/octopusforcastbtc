...
import priceupdate, publicparameters
import ordermanage, common, config, time,datetime
...
class MarketPrice(object):
    def __init__(self, market_list, save_freq_seconds=5):
        ...
        self.__coin_list = publicparameters.get_all_coin_pair_list()
    ...
    def savepricetofile(self):
        for market in self.__market_list:
            # 文件保存以月为单位一个文件
            yyyymm = str(datetime.datetime.now().year)+str(datetime.datetime.now().month)
            file_name = market+'_'+yyyymm+'.txt'
            pricefile = open(file_name, 'a')
            coin_list=self.__coin_list.get(market).split(',')
...
if __name__ == '__main__':
    pricemanage = MarketPrice(['btc38','wex'],5)
